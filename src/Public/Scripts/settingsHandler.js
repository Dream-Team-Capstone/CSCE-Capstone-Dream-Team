...
function clearAllSettings() {
  ...
  const textElements = document.querySelectorAll(
    "h2, h3, h4, h5, h6, p, span, li, a, button"
  );
...
function updateFormControls(settings) {
  ...
  if (colorContrastToggle)
    colorContrastToggle.checked = settings.highContrast === true;
...
async function saveSettings(settings) {
  ...
  if (isLoggedIn) {
    try {
      ...
      return {
        success: false,
        message: "Failed to save settings to your account.",
      };
    }
  } else {
    return {
      success: true,
      message:
        "Settings saved to this browser only. Log in to save settings to your account.",
    };
  }
}
